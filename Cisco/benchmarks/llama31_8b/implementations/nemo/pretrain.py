...
def get_data(config):
    ...
    train_datasets = sum(
        [
            ["10", f"/preproc_data/c4-train.en_{idx}_text_document"]
            for idx in [6]
        ],
        [],
    )
...
def get_model_with_precision(config, tokenizer):
    ...
    if base_config == "8b":
        ...
        filtered_kwargs = {
            ...
            if hasattr(Llama31Config8B, parameter)
        }
        ...
        base_llama_config = Llama31Config405B(seq_length=seq_length)
    else:
    ...
    base_llama_config.megatron_amp_O2 = config.model.megatron_amp_O2
...
def get_overlap_callback(config):
    ...
    print("TP COMM OVERLAP: ", ub_tp_comm_overlap)
...
def log_hyperparams(config):
    mllogger.mlperf_submission_log("llama31_8b")
    ...
    logging_configs = {
        ...
        mllogger.constants.TRAIN_SAMPLES: config.trainer.max_steps * config.model.global_batch_size,
        mllogger.constants.EVAL_SAMPLES: 1024,
        mllogger.constants.INIT_CHECKPOINT_STEP: config.model.custom.init_global_step,
        # Optimizers
        mllogger.constants.OPT_NAME: "adamw",
        ...
        mllogger.constants.OPT_LR_DECAY_STEPS: config.model.optim.sched.max_steps_for_lr_sched,
