...
_cont_mounts=("--volume=${DATADIR}:/data" "--volume=${MODEL}:/model/" "--volume=$(pwd):/workspace/code" "--volume=$(pwd)/../../AMD:/workspace/AMD" "--volume=$(pwd)/../../utilities:/workspace/utilities" "--volume=${LOGDIR}:/results")
...
for _experiment_index in $(seq 1 "${NEXP}"); do
    ...
    (
        ...
        echo 'launching experiment using:'  ${_config_env[@]} ${_cont_name} bash ./run_and_time.sh
        docker exec ${_config_env[@]} --env=HYDRA_FULL_ERROR ${_cont_name} bash ./run_and_time.sh
    ) | grep --line-buffered -v "connected peer ranks" | tee ${_current_log_file}

    if [ "${CHECK_COMPLIANCE}" -eq 1 ]; then
        docker exec "${_config_env[@]}" "${_cont_name}"  \
             python3 -m mlperf_logging.compliance_checker --usage training \
             --ruleset "${MLPERF_RULESET}"                                 \
             --log_output "/results/compliance_${_experiment_index}.out"           \
