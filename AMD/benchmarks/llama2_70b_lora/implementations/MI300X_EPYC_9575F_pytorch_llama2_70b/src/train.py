...
py_logging.getLogger('megatron.core.distributed.param_and_grad_buffer').setLevel(LOG_LEVEL)
...
def main(cfg) -> None:
    ...
    if int(os.environ.get("ENABLE_MEMORY_PROFILING", 0)) == 1:
        ...
        from callbacks.memory_callbacks import MemoryProfilerCallback, MAX_NUM_OF_MEM_EVENTS_PER_SNAPSHOT
    ...
    if os.getenv("PROFILER") == 'torchprof':
        ...
        with torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(TORCHPROF_OUTPUT_DIR),
            with_stack=True,
            record_shapes=True,
