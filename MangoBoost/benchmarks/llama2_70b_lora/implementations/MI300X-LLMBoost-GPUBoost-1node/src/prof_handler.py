...
TORCHPROF_OUTPUT_DIR = os.getenv(
    "TORCHPROF_OUTPUT_DIR", "/workspace/code/runs/torchprof"
)
...
TOTAL_WARMUP_STEPS = (
    TorchProfConfig.skip_first + TorchProfConfig.wait + TorchProfConfig.warmup
)
...
def trace_handler(prof):
    ...
    with open(save_path, "w") as f:
        output = prof.key_averages(group_by_input_shape=True).table(
            sort_by="self_cuda_time_total", row_limit=TORCHPROF_MAXROWS
        )
        f.write(output)
    ...
    prof.export_chrome_trace(
        f"/{TORCHPROF_OUTPUT_DIR}/trace_{prof.step_num}_{uuid.uuid4()}.json"
    )
