...
def run_training_warmup(trainer, warmup_train_steps, warmup_validation_steps):
    ...
    barrier()
    logging.info(
        f"Finished training warmup: {time.time() - start}s. Starting validation warmup"
    )
    ...
    if warmup_validation_steps > 0:
        ...
        for _ in range(warmup_validation_steps):
            trainer.model.validation_step(
                trainer.model.get_synthetic_input(validation=True)
            )
...
class CustomCallback(Callback):
    def __init__(self, cfg, mllogger):
        ...
        self.log_freq = int(os.environ.get("TRAINING_LOSS_LOG_FREQ", 1000))

        self.torchprof = None
        self.rpd = None
        self.rpd_status = None
        self.pi_profiler = None
        ...
        if self.profiler == "pyinstrument":
            logging.info("Using pyinstrument profiler")
            from pyinstrument import Profiler as PIProfiler

            self.pi_profiler = PIProfiler()
            self.pi_profiler.start()

        # training loss logging
        self.losses = []
        self.print_every_n_batches = int(os.environ.get("LOG_FREQ", 10))
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if (
            self.rpd
            and trainer.global_step >= TOTAL_WARMUP_STEPS
            and not self.rpd_status
        ):
            self.rpd.start()
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        if pl_module.cfg.custom.warmup:
            run_training_warmup(
                trainer,
                pl_module.cfg.custom.warmup_train_steps,
    ...
    def on_train_end(self, trainer, pl_module):
        if self.rpd and self.rpd_status == "running":
            ...
            self.rpd_status = "finished"
        if self.pi_profiler:
            self.pi_profiler.stop()
            self.pi_profiler.write_html(
                path=f"/workspace/code/pyinstrument_{trainer.global_step}_steps_rank_{get_rank()}.html",
                timeline=True,
                show_all=True,
            )
...
class MetricsLogger(Logger):
    ...
    def log_hyperparams(self, params, *args, **kwargs):
        ...
        self.mllogger.mlperf_submission_log(
            benchmark="llama2_70b_lora", num_nodes=self.cfg.trainer.num_nodes
        )
        self.mllogger.event(
            key=constants.SEED,
        ...
        self.mllogger.event(
            key="lora_rank", value=self.cfg.model.peft.lora_tuning.adapter_dim
        )
        self.mllogger.event(
            key="lora_alpha", value=self.cfg.model.peft.lora_tuning.alpha
        )
